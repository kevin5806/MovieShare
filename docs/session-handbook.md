...
- March 12, 2026: moved release validation onto a Node job container on the self-hosted runner so Postgres service networking stays reliable there, switched the automatic candidate/release path to native `linux/arm64`, and standardized day-to-day branch work on `kevin`
