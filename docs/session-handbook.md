...
- self-hosted workflow checkout should stay shell-based and fetch advertised refs (`refs/heads/...`, `refs/pull/.../head`, `refs/pull/.../merge`) using `http.extraheader` with `GITHUB_TOKEN`; URL-embedded credentials, anonymous fetches, and raw SHA fetches have each been unreliable on this runner
