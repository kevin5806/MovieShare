...
- self-hosted workflow checkout should stay shell-based and fetch advertised refs (`refs/heads/...`, `refs/pull/.../head`, `refs/pull/.../merge`) using `http.extraheader` with `GITHUB_TOKEN`; clear inherited `GIT_CONFIG_*` and `http.https://github.com/.extraheader` state first, because this runner has been carrying duplicate auth headers between jobs
...
- March 12, 2026: added explicit cleanup of inherited `GIT_CONFIG_*` / GitHub extraheader state on the self-hosted runner because jobs were failing with `Duplicate header: Authorization` even after moving to header-authenticated fetches
